...
generate_caresheet <- function(row, cs_url = "", qr_b64 = NULL) {
  ...
  moult_block <- if (!is.na(row$moult_photo) && nchar(row$moult_photo) > 0 &&
    ...
    paste0(
      ...
      '<strong>Sex confirmed: ', he(row$sex), '</strong></figcaption></figure>'
    )
  } else {
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "add_edit",
              fluidRow(
                box(title = "Add New Specimen", width = 6, status = "success",
                    textInput("in_id", "Specimen ID (Auto-generated)", placeholder = "Calculated on save"),
                    textInput("in_species", "Species"),
                    numericInput("in_dls", "Diagonal Legspan (DLS) in inches", value = 0.25, min = 0, step = 0.1),
                    selectInput("in_sex", "Sex", choices = c("Unknown","Female","Male")),
                    actionButton("btn_add", "Add Specimen", icon = icon("plus"), class = "btn-success")),
                
                box(title = "Species-Wide Care (Applies to all of this species)", width = 6, status = "info",
                    selectInput("hus_species_select", "Select Species to Edit Care", choices = NULL),
                    # ... [Include the existing husbandry inputs here (temp, humidity, etc.)] ...
                    actionButton("btn_save_species_hus", "Save Care Rules", class = "btn-info"))
              )
      ),
