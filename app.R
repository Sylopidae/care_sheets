...
#  Full feature set:
#    • Per-spider caresheets (self-contained HTML)
#    • Moult photo upload + base64 embedding for sex verification
#    • QR code generation (links to GitHub Pages caresheet URL)
#    • GitHub Pages base URL config (persisted to spider_config.rds)
#    • GitHub sync: git add -A / commit / push  (Windows CMD fix)
#      pushes care_sheets/ + moults/ + qrcodes/ automatically
# ============================================================
...
# ── persistent config (GitHub Pages base URL) ────────────────
...
save_moult_photo <- function(tmp_path, spider_id, species) {
  ...
  name <- paste0(spider_id, "_", slug(species), ".", ext)
  dest <- file.path(MOULT_DIR, name)
...
caresheet_url <- function(spider_id, species, base_url) {
  ...
  fname    <- paste0(spider_id, "_", slug(species), ".html")
  paste0(base_url, "/", CARESHEET_DIR, "/", fname)
}

generate_qr_png <- function(spider_id, species, base_url) {
  url  <- caresheet_url(spider_id, species, base_url)
  name <- paste0(spider_id, "_", slug(species), "_qr.png")
  path <- file.path(QRCODE_DIR, name)
...
generate_caresheet <- function(row, cs_url = "", qr_b64 = NULL) {
  ...
  moult_block <- if (!is.na(row$moult_photo) && nchar(row$moult_photo) > 0 &&
    ...
    paste0(
      ...
      '<img src="', b64, '" alt="Moult - sex verification" ',
      ...
      '&#128247; Moult photograph provided for sex verification.<br>',
      '<strong>Sex confirmed: ', he(row$sex), '</strong>',
      '</figcaption></figure>'
    )
  } else {
  ...
  qr_block <- if (!is.null(qr_b64) && nchar(cs_url) > 0) {
    paste0(
      ...
      'style="width:160px;height:160px;border:1px solid #ddd;border-radius:6px;">',
      '<p style="margin:8px 0 0;font-size:.8em;color:#555;">',
      'Scan to view the live online caresheet<br>',
  ...
  sex_col  <- switch(row$sex %||% "Unknown", Female = "#e83e8c", Male = "#007bff", "#6c757d")
  temp_str <- if (!is.na(row$temp_low_f) && !is.na(row$temp_high_f))
    paste0(row$temp_low_f, "\u00b0F \u2013 ", row$temp_high_f, "\u00b0F") else "Not specified"
  humidity_str <- if (!is.na(row$humidity_pct)) paste0(row$humidity_pct, "%") else "Not specified"
  age_str <- tryCatch({
...
<style>
  ...
  .badge{display:inline-block;padding:4px 14px;border-radius:20px;font-size:.85em;
         font-weight:700;color:#fff;background:', sex_col, ';}
  .id-chip{background:rgba(255,255,255,.15);border-radius:6px;padding:4px 10px;
           font-size:.82em;font-family:monospace;}
  table{width:100%;border-collapse:collapse;font-size:.95em;}
  .section{background:#fff;border:1px solid #e0e0e0;border-radius:8px;
           margin-bottom:20px;overflow:hidden;}
  .section-header{background:#2d2d44;color:#fff;padding:8px 14px;font-weight:700;
                  font-size:.9em;letter-spacing:.04em;text-transform:uppercase;}
  .care-notes{padding:12px;line-height:1.65;white-space:pre-wrap;font-size:.93em;}
...
<div class="header">
  <div>
    <div class="id-chip">Spider Farm \u2013 ID #', row$id, '</div>
...
<div class="section" style="margin-top:0;border-radius:0;">
  <div class="section-header">&#128241; Online Caresheet &amp; QR Code</div>
  ', qr_block, '
</div>
<div class="section">
  <div class="section-header">&#9888; Sex Verification \u2013 Moult Photograph</div>
  <div style="padding:16px;">', moult_block, '
    ...
    book lungs on the underside of the abdomen.</p>
  </div>
</div>
<div class="section">
  <div class="section-header">&#128203; Spider Identity</div>
  <table>',
         ...
         '</table></div>
<div class="section">
  <div class="section-header">&#127968; Husbandry Requirements</div>
  <table>',
         field("Temperature",    temp_str),
         field("Humidity",       humidity_str),
         ...
         '</table></div>
<div class="section">
  <div class="section-header">&#127829; Feeding &amp; Water</div>
  <table>',
         ...
         '</table></div>
<div class="section">
  <div class="section-header">&#128221; Additional Care Notes</div>
  <div class="care-notes">',
         if (!is.na(row$care_notes) && nchar(trimws(row$care_notes)) > 0)
           he(row$care_notes)
         else '<span style="color:#888;font-style:italic;">No additional notes provided.</span>',
         '</div></div>
<div class="section">
  <div class="section-header">&#10084; Health Status at Sale</div>
  <table>',
         field("Health", row$health),
         field("Notes",  row$notes),
         '</table></div>
<footer>
  Spider Farm &bull; ID #', row$id, ' &bull; <em>', he(row$species), '</em> &bull;
  Generated ', format(Sys.time(), "%Y-%m-%d %H:%M:%S"), '<br>
  This document is provided for the buyer\u2019s personal reference.
</footer>
</body></html>')
}

# ── save caresheet + QR to disk ──────────────────────────────
save_caresheet <- function(row, base_url = "") {
  ...
  if (nchar(trimws(base_url)) > 0) {
    ...
    qr_path <- tryCatch(generate_qr_png(row$id, row$species, base_url),
                        error = function(e) { message("QR error: ", e); NULL })
    if (!is.null(qr_path)) qr_b64 <- file_to_base64(qr_path, "image/png")
  }
  html    <- generate_caresheet(row, cs_url = cs_url, qr_b64 = qr_b64)
  fname   <- paste0(row$id, "_", slug(row$species), ".html")
  cs_path <- file.path(CARESHEET_DIR, fname)
    ...
    tabItems(
      ...
      tabItem(tabName = "add_edit",
              fluidRow(
                box(title = "Add New Spider", width = 6, status = "success",
                    textInput("in_species",   "Species",      placeholder = "e.g. Brachypelma hamorii"),
              ...
              box(title = "Husbandry Details", width = 8, status = "info",
               ...
               All fields are embedded into the generated caresheet."),
                  ...
                  fluidRow(
                    ...
                    column(6,
                           textInput("hus_enc_size",  "Enclosure Size", placeholder = 'e.g. 8"x8"x12"'),
                           ...
                           numericInput("hus_size",     'Adult Size (DLS, in")', value=NA, min=0, step=0.25),
                           numericInput("hus_lifespan", "Typical Lifespan (yrs)", value=NA, min=0, step=0.5))
                  ),
                  textAreaInput("hus_care_notes", "Additional Care Notes", rows = 4,
                                placeholder = "Moulting notes, communal tolerance, defensive triggers..."),
      ...
      tabItem(tabName = "caresheets",
              fluidRow(
                box(title = "Generate Caresheet", width = 5, status = "success",
                    p("Builds a self-contained HTML caresheet with the moult photo and QR code",
                      "embedded. Set your GitHub Pages URL in the",
                      strong("GitHub Sync"), "tab first."),
      ...
      tabItem(tabName = "feeding",
              box(title = "Log a Feeding", width = 5, status = "info",
                  ...
                  actionButton("btn_feed", "Log Feeding", icon = icon("check"), class = "btn-info"),
      ...
      tabItem(tabName = "analytics",
              fluidRow(
                box(title = "Weight Distribution",     width = 6, status = "primary", plotOutput("plot_weight")),
                box(title = "Health Status Breakdown", width = 6, status = "primary", plotOutput("plot_health"))
              ),
              fluidRow(
                box(title = "Species Count", width = 6, status = "primary", plotOutput("plot_species")),
                box(title = "Sex Ratio",     width = 6, status = "primary", plotOutput("plot_sex"))
              )
      ),
      
      tabItem(tabName = "github",
              fluidRow(
                box(title = "GitHub Pages URL", width = 12, status = "warning",
                    p("This URL is encoded into every QR code and caresheet link.",
                      "Format:", code("https://username.github.io/repo-name")),
                    fluidRow(
                      column(9,
                             textInput("pages_url", NULL, value = CFG$pages_base_url,
                                       placeholder = "https://username.github.io/repo-name",
                                       width = "100%")),
                      column(3,
                             actionButton("btn_save_url", "Save URL",
                                          icon = icon("save"), class = "btn-warning btn-block"))
                    ),
                    verbatimTextOutput("out_url_status"))
              ),
              fluidRow(
                box(title = "Push to GitHub", width = 8, status = "success",
                    p("Commits and pushes all local changes including",
                      code("spider_data.csv"), ",", code("care_sheets/"), ",",
                      code("moults/"), ", and", code("qrcodes/"), "."),
                    br(),
                    textInput("sync_msg", "Commit Message",
                              value = paste0("update caresheets + moults + qrcodes ",
                                             format(Sys.time(), "%Y-%m-%d %H:%M:%S"))),
                    textInput("sync_repo", "Repo Path (blank = app directory)", value = REPO_PATH),
                    fluidRow(
                      column(6,
                             actionButton("btn_gen_then_sync",
                                          "Generate All Caresheets + QR Codes then Push",
                                          icon = icon("rocket"), class = "btn-success btn-lg")),
                      column(6,
                             actionButton("btn_sync_only", "Push Current Files (skip regen)",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_save_url, {
    rv$cfg$pages_base_url <- trimws(input$pages_url)
    save_config(rv$cfg)
    output$out_url_status <- renderText(
      paste0("\u2714 Saved: ", rv$cfg$pages_base_url,
             "\nAll new caresheets and QR codes will use this URL."))
  })
  ...
  output$tbl_all <- renderDT({
    df_show <- rv$df[, intersect(DISP, names(rv$df))]
    df_show$moult_photo <- ifelse(
      !is.na(df_show$moult_photo) & nchar(df_show$moult_photo) > 0 &
        file.exists(df_show$moult_photo), "\u2705 Attached", "\u274c None")
    datatable(df_show, selection="single", rownames=FALSE,
  ...
  observeEvent(input$btn_add, {
    ...
    nr <- as.data.frame(setNames(as.list(rep(NA_character_, length(SPIDER_COLS))),
    ...
    output$out_add_status <- renderText(paste0("\u2714 Added: ", nr$species, " (ID ", nr$id, ")"))
  })
  ...
  observeEvent(input$btn_load_edit, {
    ...
    output$out_edit_status <- renderText("Record loaded \u2013 adjust fields then click Save.")
  })
  ...
  observeEvent(input$btn_moult_upload, {
    ...
    dest <- save_moult_photo(input$moult_file$datapath, input$moult_id, rv$df[idx,"species"])
    rv$df[idx,"moult_photo"] <- dest
    rv$df[idx,"timestamp"]   <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    save_data(rv$df)
    output$out_moult_status <- renderText(paste0(
      "\u2714 Photo saved: ", dest,
      "\nTip: regenerate the caresheet to embed the updated image."))
  })
  ...
  observeEvent(input$btn_moult_preview, {
    ...
    output$moult_preview_ui <- renderUI({
      ...
      tags$div(
        tags$p(strong("Species: "), rv$df[idx,"species"], " | ", strong("Sex: "), rv$df[idx,"sex"]),
        tags$img(src=b64, style="max-width:100%;max-height:420px;border-radius:6px;border:2px solid #ccc;"),
        tags$p(style="color:#555;font-size:.85em;margin-top:6px;", "Saved as: ", path)
      )
    })
  })
  
  output$tbl_moults <- renderDT({
    df_m <- rv$df[!is.na(rv$df$moult_photo) & nzchar(rv$df$moult_photo),]
    if (nrow(df_m)==0) return(datatable(data.frame(Message="No moult photos attached yet.")))
    show <- df_m[, c("id","species","sex","moult_photo")]
    show$file_exists <- file.exists(show$moult_photo)
  ...
  do_generate_cs <- function(spider_id) {
    ...
    if (is.null(res)) return(list(ok=FALSE, msg=paste("Error for ID", spider_id)))
    qr_note <- if (!is.null(res$qr_path))
      paste0("\n   QR \u2192 ", res$qr_path, "\n   URL: ", res$cs_url)
    else "\n   (No QR \u2014 set GitHub Pages URL first)"
    html_preview <- generate_caresheet(row, cs_url=res$cs_url,
                                       qr_b64=if (!is.null(res$qr_path)) file_to_base64(res$qr_path, "image/png") else NULL)
    list(ok=TRUE, res=res, html=html_preview,
  ...
  observeEvent(input$btn_gen_all_cs, {
    ...
    output$out_cs_bulk_status <- renderText(
      paste0("Done (", length(ids), " spiders):\n", paste(msgs, collapse="\n")))
  })
  
  output$tbl_caresheets <- renderDT({
    input$btn_refresh_cs
    files <- list.files(CARESHEET_DIR, pattern="\\.html$", full.names=FALSE)
    if (length(files)==0) return(datatable(data.frame(File="No caresheets yet.")))
    info <- file.info(file.path(CARESHEET_DIR, files))
    datatable(data.frame(File=files, Size_KB=round(info$size/1024,1),
